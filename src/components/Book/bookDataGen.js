const bookDataGen = (chapters) => {
  ...
  const categories = ['Fiction', 'Adventure', 'Drama', 'Novell', 'Suspense', 'True History', 'Politic', 'Technical'];
  const bookCategory = Math.floor(Math.random() * categories.length);

  const data = new Map([
    ['category', categories[bookCategory]],
